...
test('choose sauce labs bike light test', async ({ page }) => {
  await page.goto('https://www.saucedemo.com/');
  await page.locator('[data-test="username"]').fill('standard_user');
  await page.locator('[data-test="password"]').fill('secret_sauce');
  await page.locator('[data-test="login-button"]').click();
  await page.locator('[data-test="item-0-title-link"]').click();
  const pageToCheck = await page.locator('[data-test="inventory-item-name"]').textContent();
  expect(pageToCheck).toEqual('Sauce Labs Bike Light');
